...
class Monky {
   ...
   * @param {Object} options Model options, optional
   */
  ref(model, path, options) {
    return new MonkyRef(model, path, options)
  }
  ...
  * @param {Function}  cb          Callback
  ...
  set(model, instance, path, value, saveRelated, cb) {
    ...
    const handleMonkyRef = (monkyRef, callback) => {
      let fn = self.build
      if (saveRelated) fn = self.create

      fn.bind(self)(monkyRef.model, monkyRef.options, (err, result) => {
        if (err) return cb(err)

        if (_.isString(monkyRef.path)) {
          _.set(instance, path, result[monkyRef.path])
        } else {
    ...
    if (_.isFunction(value)) {
      ...
      // For Mongoose 6+ compatibility: when setting an array of MonkyRefs,
      // we must set the whole array via instance.set() to avoid casting
      // individual elements to ObjectIds. Only applies to "simple" arrays
      // where elements are refs/models/ids/primitives.
      const hasPlainObjects = value.some(sub => sub && _.isPlainObject(sub))
      const hasRefs = value.some(sub => sub instanceof MonkyRef || (sub && 'model' === sub.constructor.name))

      if (!hasPlainObjects && hasRefs) {
        ...
        _each(value, (sub, eachDone) => {
          ...
          if (sub instanceof MonkyRef) {
            let fn = self.build
            if (saveRelated) fn = self.create
            fn.bind(self)(sub.model, sub.options, (err, result) => {
              if (err) return eachDone(err)
              resolved[idx] = _.isString(sub.path) ? result[sub.path] : result
              eachDone(null)
            })
          } else if (sub && 'model' === sub.constructor.name) {
            resolved[idx] = sub
            eachDone(null)
          } else if (sub && 'ObjectID' === sub.constructor.name) {
            resolved[idx] = sub
            eachDone(null)
          } else {
            resolved[idx] = sub
          ...
          if (err) return cb(err)
          instance.set(path, resolved)
          resolved.forEach((result, i) => {
            if (result && 'model' === result.constructor.name) {
              instance.populated(`${path}.${i}`, result)
            }
          })
          cb(null)
        })
...
class MonkyRef {
  constructor(model, path, options) {
    ...
    // If path is omitted and the second param is an object
    // i.e. it's a set of model attributes
    if (_.isObject(path)) {
      this.path = null
      this.options = path
    }
